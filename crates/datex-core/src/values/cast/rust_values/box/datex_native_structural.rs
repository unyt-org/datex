...
use crate::prelude::*;
use crate::preludes::derive::HasClassification;
...
impl<T: DatexNativeStructural + GetDatexType + HasClassification> DatexNativeStructural for Box<T> {}

/// If `T` implements [DatexNativeOnlyStructural], then `Box<T>` also implements [DatexNativeOnlyStructural].
impl<T: DatexNativeOnlyStructural + GetDatexType + HasClassification> DatexNativeOnlyStructural for Box<T> {}