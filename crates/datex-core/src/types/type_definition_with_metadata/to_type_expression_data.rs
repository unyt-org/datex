use crate::{
    ast::{spanned::Spanned, type_expressions::TypeExpressionData},
    prelude::*,
    ...
    types::type_definition_with_metadata::{
        LocalMutability, LocalOwnership, LocalReferenceMutability,
...
impl ToTypeExpressionData for TypeDefinitionWithMetadata {
    fn to_type_expression_data(&self) -> TypeExpressionData {
        ...
        if matches!(
            ...
            type_expr = TypeExpressionData::Mut(Box::new(
                type_expr.with_default_span(),
            ));
        }

        // add "shared" prefix
        if matches!(self.metadata, TypeMetadata::Shared { .. }) {
            type_expr = TypeExpressionData::Shared(Box::new(
                type_expr.with_default_span(),
            ));
        }
        ...
        if matches!(
            self.metadata,
            TypeMetadata::Local {
                ownership: LocalOwnership::Referenced(
                    LocalReferenceMutability::Mutable
                ),
                ..
            }
        ) {
            type_expr = TypeExpressionData::RefMut(Box::new(
                type_expr.with_default_span(),
            ));
        } else if matches!(
            self.metadata,
            ...
            type_expr = TypeExpressionData::Ref(Box::new(
                type_expr.with_default_span(),
            ));
        }
