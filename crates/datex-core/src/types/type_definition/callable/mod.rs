...
pub mod serde_dif;
use crate::{
    prelude::*, traits::apply::ApplyArgument,
    types::traits::type_match::TypeSatisfiesValueContainer,
};
...
impl Display for InvalidArgumentError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            InvalidArgumentError::InvalidType {
                arg_name,
                expected,
                provided,
            } => {
                write!(
                    ...
                    arg_name, expected, provided
                )
            }
            InvalidArgumentError::InvalidArgumentCount {
                expected,
                provided,
            } => {
                write!(
            ...
            if !param_type.satisfies_value_container(&arg.value) {
                return Err(InvalidArgumentError::InvalidType {
                    ...
                    provided: Type::Definition(
                        arg.value.actual_container_type(),
                    ),
                });
            }
        }
        ...
        if let Some((rest_param_name, rest_param_type)) = &self.rest_parameter {
            for (i, arg) in args[self.parameters.len()..].iter().enumerate() {
                if !rest_param_type.satisfies_value_container(&arg.value) {
                    return Err(InvalidArgumentError::InvalidType {
                        arg_name: rest_param_name
                            .clone()
                            .unwrap_or_else(|| format!("rest_arg_{}", i)),
