...
impl PointerAddress {
    ...
    pub fn builtin_bytes(&self) -> Option<&[u8; 3]> {
        if let PointerAddress::External(ExternalPointerAddress::Builtin(addr)) =
            self
        {
            Some(&addr.0)
        } else {
