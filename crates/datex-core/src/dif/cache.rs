...
impl From<UnexpectedSharedContainerOwnershipError> for CacheValueRetrievalError {
...
impl DIFSharedContainerCache {
    ...
    pub fn try_take_owned_shared_container(&mut self, pointer_address: &PointerAddress) -> Result<OwnedSharedContainer, CacheValueRetrievalError> {
                ...
                match container {
                    SharedContainer::Owned(container) => {
                        ...
                        match self.values.insert(pointer_address.clone(), SharedContainer::Referenced(container.derive_with_max_mutability())) {
                            Some(SharedContainer::Owned(owned_container)) => Ok(owned_container),
                            _ => unreachable!(),
                        }
                    }
    ...
    pub fn try_get_shared_container(&self, pointer_address: &PointerAddress) -> Result<&SharedContainer, ValueNotFoundInCacheError> {
        self.values.get(pointer_address).ok_or(ValueNotFoundInCacheError)
    }
    ...
    pub fn try_get_shared_container_mutable_reference(&mut self, pointer_address: &PointerAddress) -> Result<ReferencedSharedContainer, CacheValueRetrievalError> {
        Ok(
    ...
    /// If the container for the address is not in the cache, a [ValueNotFoundInCacheError] is returned.
    pub fn try_get_shared_container_immutable_reference(&mut self, pointer_address: &PointerAddress) -> Result<ReferencedSharedContainer, ValueNotFoundInCacheError> {
        Ok(
            self.values.get(pointer_address)
                .ok_or(ValueNotFoundInCacheError)?
    ...
    pub fn try_get_shared_container_with_ownership(&mut self, pointer_address: &PointerAddress, ownership: SharedContainerOwnership) -> Result<SharedContainer, CacheValueRetrievalError> {
        match ownership {
            ...
            SharedContainerOwnership::Referenced(ReferenceMutability::Immutable) => Ok(self.try_get_shared_container_immutable_reference(pointer_address).map(SharedContainer::Referenced)?),
        }
    }
}
