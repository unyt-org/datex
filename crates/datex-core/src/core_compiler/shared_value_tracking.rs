...
impl<'a> SharedValueTracking<'a> {
    ...
    fn register_shared_value_with_parents(
        ...
        if let Some(container) =self.update_container_ownership_if_exists(shared_container) {
...
use super::*;
    ...
    fn assert_child(
        ...
        assert_matches!(tracked_value(tracking, container), &TrackedValueMetadata::Child {..});
    }
