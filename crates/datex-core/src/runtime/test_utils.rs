use crate::{
    network::com_hub::test_utils::couple_com_hubs,
...
{
    let runner_a = RuntimeRunner::new(RuntimeConfig::new_with_endpoint(
        endpoint_a.clone(),
    ));
    let runner_b = RuntimeRunner::new(RuntimeConfig::new_with_endpoint(
        endpoint_b.clone(),
    ));
    ...
    runner_a
        .run(async move |runtime_a| {
            runner_b
                .run(async move |runtime_b| {
                    // couple the com hubs of the two runtimes
                    couple_com_hubs(runtime_a.com_hub(), runtime_b.com_hub())
                        .await;
    ...
    assert!(
        end_reached.get(),
        "The provided app logic did not finish executing"
    );
}
