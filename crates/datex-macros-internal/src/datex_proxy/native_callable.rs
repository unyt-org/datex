...
use quote::quote;
use syn::{fold, ExprClosure, ImplItemFn, ItemFn, Signature, Type};
use syn::fold::Fold;
use crate::utils::get_datex_core_crate_name;

pub fn generate_native_callable_from_impl_fn(function: &ImplItemFn, self_type: &Type) -> TokenStream {
...
pub fn generate_native_callable_from_closure(function: &ExprClosure) -> TokenStream {
    todo!()
    // generate_native_callable(
...
pub fn generate_native_callable(
    ...
    let return_type_tokens = match return_type {
        Some(ref ty) => quote! { Some(Box::new(<#ty as DatexProxyTypes>::datex_type(cache))) },
        None => quote! { None },
    };
    ...
    let has_mutable_inputs = sig.inputs.iter().any(|param| {
        match param {
